const passwordRegexes = {
	...
	lowerUpperDigitSpecial: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#]).{8,}$/,
	strictAlphanumeric: /^(?=.{8,16}$)(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]+$/,
	strongNoWhitespace: /^(?=.{12,}$)(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*\W)(?!.*\s).+$/,
...
const emailRegexes = {
	...
	noNumericOnlyLocal: /^(?!\d+$)[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/,
};
...
// Example wrapper for account registration
